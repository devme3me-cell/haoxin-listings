  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 lg:px-12">
          ...
          <nav className="hidden lg:flex items-center gap-10">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-sm tracking-wider hover:text-warm-gold transition-colors duration-300"
              >
                {item.name}
              </a>
            ))}
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <nav className="lg:hidden absolute top-full left-0 right-0 bg-background/98 backdrop-blur-md border-t border-border shadow-lg animate-slide-up">
            <div className="container py-8 space-y-6">
              {navItems.map((item) => (
                <a
                  ...
                  className="block text-lg tracking-wider hover:text-warm-gold transition-colors"
                >
                  {item.name}
                </a>
              ))}
